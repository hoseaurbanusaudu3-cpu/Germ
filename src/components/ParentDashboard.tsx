...
export function ParentDashboard({ onLogout }: ParentDashboardProps) {
  ...
  // Get parent's children from SchoolContext
  ...
  const children: Child[] = parentStudents.map(s => ({
    ...
    class: s.className,
    balance: 0, // This will be calculated from fee balance
    photo: "",
    totalFee: 0,
  }));
              ...
              <div className="grid lg:grid-cols-2 gap-4">
                <Card className="rounded-lg bg-white border border-[#E5E7EB] shadow-clinical">
                  ...
                  <CardContent className="space-y-3 p-5 pt-5">
                    {[
                      { title: "Mid-term results available", time: "2 hours ago", type: "info" },
                      { title: "Parent-Teacher meeting scheduled", time: "1 day ago", type: "warning" },
                      { title: "Fee payment reminder", time: "3 days ago", type: "alert" },
                    ].map((notification, index) => (
                      <div key={index} className="flex items-start gap-3 p-3 bg-[#F9FAFB] rounded-lg border border-[#E5E7EB] hover:border-[#3B82F6] hover:shadow-clinical transition-all">
                        <div className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${
                          notification.type === 'info' ? 'bg-[#3B82F6]' :
                          notification.type === 'warning' ? 'bg-[#F59E0B]' : 'bg-[#EF4444]'
                        }`} />
