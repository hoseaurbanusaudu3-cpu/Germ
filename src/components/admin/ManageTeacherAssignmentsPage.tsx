...
export function ManageTeacherAssignmentsPage() {
  ...
  const handleSaveAssignments = () => {
    ...
    toast.success(`${selectedAssignments.length} assignment(s) added for ${teacher.firstName} ${teacher.lastName}`);
  ...
  const handleAssignClassTeacher = () => {
    ...
    // Validate assignment (max 3 classes)
    const validation = validateClassTeacherAssignment(selectedTeacherId, parseInt(selectedClassForTeacher));
  ...
  const handleRemoveClassTeacher = (classId: number) => {
    ...
    const remainingAssignments = classes.filter(
      c => c.classTeacherId === teacherId && c.id !== classId
    );

    // If no more class teacher assignments, update teacher
        ...
        <DialogContent className="max-w-4xl rounded-xl bg-white border border-gray-200 text-gray-900 max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-500">Selected: {selectedAssignments.length} combination(s)</p>
            </div>
